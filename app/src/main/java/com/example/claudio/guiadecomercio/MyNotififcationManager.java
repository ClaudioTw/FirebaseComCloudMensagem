package com.example.claudio.guiadecomercio;

import android.app.NotificationManager;
import android.app.PendingIntent;
import android.content.Context;
import android.content.Intent;
import android.support.v4.app.NotificationCompat;

/**
 * Created by claudio on 19/02/18.
 */

public class MyNotififcationManager {

    private Context mCtx;
    private  static  MyNotififcationManager mInstance;

    private MyNotififcationManager (Context context){

        mCtx = context;

    }

    public static synchronized  MyNotififcationManager getInstance(Context context){
        if (mInstance == null ){
            mInstance  = new MyNotififcationManager(context);
                    }

                    return  mInstance;
    }

    public void displayNotification(String title, String body){
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(mCtx, Constants.CHANNEL_ID)
                .setSmallIcon(R.drawable.meulogo)
                .setContentTitle(title)
                .setContentText(body);

        Intent intent = new Intent(mCtx, Main2Activity.class);

        PendingIntent pendingIntent = PendingIntent.getActivity
                (mCtx, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);

    mBuilder.setContentIntent(pendingIntent);
        NotificationManager mNotificationManager = (NotificationManager)
                mCtx.getSystemService(Context.NOTIFICATION_SERVICE);

        if(mNotificationManager !=null){
            mNotificationManager.notify(1, mBuilder.build());

        }




    }

}
